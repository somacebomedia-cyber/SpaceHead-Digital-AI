...
export default function GoogleDriveManager({ 
  ...
  /**
   * Sync all videos/images in a specified folder ID or URL (Pathway 1)
   */
  const handleSyncFolderId = async (folderIdOrUrl: string) => {
    ...
    try {
      ...
      const data = await listPublicDriveFiles(folderId);
      const fetchedFiles = data.files || [];

      // Filter for files with video or image mime types/extensions
      const importableFiles = fetchedFiles.filter(f => {
        const mime = (f.mimeType || "").toLowerCase();
        const name = (f.name || "").toLowerCase();
        const isVideo = mime.startsWith("video/") || name.endsWith(".mp4") || name.endsWith(".mov") || name.endsWith(".webm");
        const isImage = mime.startsWith("image/") || name.endsWith(".png") || name.endsWith(".jpg") || name.endsWith(".jpeg") || name.endsWith(".webp");
        return isVideo || isImage;
      });

      if (importableFiles.length === 0) {
        throw new Error("No video or image files found in the specified Google Drive folder. Ensure the folder is public and contains supported formats.");
      }
      ...
      for (const file of importableFiles) {
        ...
        const imgUrl = `https://lh3.googleusercontent.com/d/${file.id}=w800`;
        ...
        if (isVideo) {
          ...
          await onImportProject({
            ...
            description: `High-converting bespoke graphic design post crafted for ultimate visual engagement and premium aesthetics. Part of our creative concepts collection, synchronized dynamically.`,
            tags: ["Graphic Design", "Brand Assets", "Drive Sync"],
            demoUrl: file.webViewLink || `https://drive.google.com/file/d/${file.id}/view`,
            imageUrl: imgUrl,
            isPublic: true,
            createdAt: Date.now(),
            userId: "system"
      ...
      setSuccessMsg(`Successfully synced and imported ${count} new portfolio asset(s) into your Showcase Portfolio!`);
      setFolderSyncInput("");
    } catch (err: any) {
      setErrorMsg(err.message || "Failed to sync folder. Ensure the folder is public or shared with access.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div id="drive-manager" className="bg-white border border-slate-150 rounded-3xl p-6 sm:p-8 shadow-sm space-y-6 relative overflow-hidden">
      ...
      {/* Showcase Auto-Sync Folder Engine (Pathway 1) - ALWAYS VISIBLE */}
      <div className="bg-gradient-to-r from-orange-50/80 to-amber-50/80 border border-orange-200/50 p-5 rounded-2xl space-y-3 relative overflow-hidden shadow-sm">
        <div className="absolute top-0 right-0 w-32 h-32 bg-orange-500/5 rounded-full blur-xl pointer-events-none" />
        <div className="flex items-center space-x-2 text-orange-800">
          <RefreshCw className="w-4 h-4 animate-spin animate-duration-[12000ms]" />
          <h4 className="text-xs font-bold uppercase tracking-wider">Showcase Auto-Sync Folder Engine</h4>
        </div>
        <p className="text-xs text-slate-600 leading-relaxed max-w-2xl">
          Paste any Google Drive Folder URL or ID (like your public graphics folder) to automatically retrieve, format, and import all images and video files directly as live project cards in your showcase portfolio!
        </p>
        <form onSubmit={(e) => { e.preventDefault(); handleSyncFolderId(folderSyncInput); }} className="flex flex-col sm:flex-row gap-2 max-w-3xl">
          ...
          >
            <span>Import Folder to Portfolio</span>
          </button>
        </form>
      </div>
